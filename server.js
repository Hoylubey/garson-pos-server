// server.js (Düzeltilmiş ve logları eklenmiş hali)
...
try {
    // FIREBASE_SERVICE_ACCOUNT_KEY ortam değişkeninden yükleniyor
       ...
       `).run(username);
        ...
        connectedClients.forEach((clientInfo, clientSocketId) => {
            if (clientInfo.role === 'admin' || clientInfo.role === 'garson') {
                ...
                // Motorcu haritadan kaldırılmalı
...
io.on('connection', (socket) => {
    ...
    socket.on('registerClient', (clientInfo) => {
        ...
        if (role === 'admin' || role === 'garson') {
            ...
            // HARİTA VERİSİ GÖNDERME KISMI
            io.to(socket.id).emit('currentRiderLocations', Object.values(riderLocations));
            console.log(`[*** HARİTA YÜKLEME KONTROLÜ ***] ${username} (${role}) kullanıcısına (${socket.id}) ${Object.values(riderLocations).length} motorcu konumu gönderildi.`);
    ...
    socket.on('riderLocationUpdate', (locationData) => {
        ...
        // 2. Motorcunun 'riders' tablosunda var olup olmadığını kontrol et.
        ...
        connectedClients.forEach((clientInfo, clientSocketId) => {
            if (clientInfo.role === 'admin' || clientInfo.role === 'garson') {
                // YENİ LOG: Hata ayıklama için gönderilen veriyi logluyoruz
                console.log(`[Sunucu Yayın] 'newRiderLocation' gönderiliyor: ${clientInfo.username} -> ${JSON.stringify(riderLocations[username])}`); 
    ...
    socket.on('assignOrderToRider', async (data) => {
        ...
        try {
            ...
            if (updatedOrder) {
                ...
                // Not: 'riderId' yerine 'riderUsername' kullanmak daha güvenli olabilir.
                const riderSocketId = Array.from(connectedClients.keys()).find(key => connectedClients.get(key).username === riderUsername && connectedClients.get(key).role === 'rider');
                if (riderSocketId) {
                    ...
                    console.error(`[${new Date().toLocaleTimeString()}] Motorcu ${riderUsername} çevrimiçi değil, sipariş Socket.IO ile gönderilemedi.`);
                }
            } else {
    ...
    socket.on('disconnect', () => {
        ...
        if (clientInfo) {
            ...
            // Motorcu bağlantısı kesilirse haritadan kaldır.
            if (clientInfo.role === 'rider' && riderLocations[clientInfo.username] && riderLocations[clientInfo.username].socketId === socket.id) {
                
                const disconnectedRiderUsername = clientInfo.username;
                delete riderLocations[disconnectedRiderUsername];
                
                // Tüm admin/garson panellerine motorcunun ayrıldığını bildir (Android'de String bekleniyor)
                connectedClients.forEach((info, clientSocketId) => {
                    if (info.role === 'admin' || info.role === 'garson') {
                        io.to(clientSocketId).emit('riderDisconnected', disconnectedRiderUsername); 
                    }
                });
                console.log(`Motorcu ${disconnectedRiderUsername} haritadan kaldırıldı ve admin/garsonlara bildirildi.`);
            }
        }
    });
});
